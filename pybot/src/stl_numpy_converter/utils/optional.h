...
struct python_optional : public boost::noncopyable {
  ...
  struct optional_from_python {
    static void * convertible(PyObject * source) {
      ...
      const bpc::registration& converters(bpc::registered<T>::converters);

      if (implicit_rvalue_convertible_from_python(source,
                                                  converters)) {
        bpc::rvalue_from_python_stage1_data data =
    ...
    static void construct(PyObject * source,
      ...
      void * const storage = ((bpc::rvalue_from_python_storage<T> *)
                              data)->storage.bytes;
