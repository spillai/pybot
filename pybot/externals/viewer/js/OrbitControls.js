...
THREE.OrbitControls = function ( object, domElement ) {
    ...
    this.animationSpeed = 1.0; // 0.2 (for smooth animations)
	...
	if ( scope.object.fov !== undefined ) {
	    ...
	    var targetDistance = offset.length() * Math.tan( (
	        scope.object.fov / 2 ) * Math.PI / 180.0 );

	    // we actually don't use screenWidth, since perspective
	    // camera is fixed to screen height
	    scope.panLeft( 2 * deltaX * targetDistance / element.clientHeight );
	    scope.panUp( 2 * deltaY * targetDistance / element.clientHeight );
	} else if ( scope.object.top !== undefined ) {
	    // orthographic
	    scope.panLeft( deltaX * (scope.object.right -
	                             scope.object.left) / element.clientWidth );
	    scope.panUp( deltaY * (scope.object.top -
	                           scope.object.bottom) / element.clientHeight );
	} else {
	...
	if ( state === STATE.ROTATE ) {
	    ...
	    // rotating across whole screen goes 360 degrees around
	    scope.lookAroundLeft(2 * Math.PI * lookAroundDelta.x /
	                         element.clientWidth * scope.lookAroundSpeed);

	    // rotating up and down along whole screen attempts to go
	    // 360, but limited to 180
	    scope.lookAroundUp(2 * Math.PI * lookAroundDelta.y /
	                       element.clientWidth * scope.lookAroundSpeed);
	...
	if ( scope.enabled === false || scope.noKeys === true ||
	     scope.noPan === true ) return;
