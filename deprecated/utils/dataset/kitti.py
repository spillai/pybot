...
def kitti_stereo_calib(sequence, scale=1.0):
    seq = int(sequence)
    print('KITTI Dataset Reader: Sequence ({:}) @ Scale ({:})'.format(
        sequence, scale))
    if seq >= 0 and seq <= 2:
...
class OXTSReader(DatasetReader):
    ...
    def oxts_process_cb(self, fn):
        ...
        Rx = np.float32([1, 0, 0, 0, np.cos(rx), -np.sin(rx),
                         0, np.sin(rx), np.cos(rx)]).reshape(3, 3)
        Ry = np.float32([np.cos(ry), 0, np.sin(ry), 0, 1, 0, -
                         np.sin(ry), 0, np.cos(ry)]).reshape(3, 3)
        Rz = np.float32([np.cos(rz), -np.sin(rz), 0, np.sin(rz),
                         np.cos(rz), 0, 0, 0, 1]).reshape(3, 3)
...
class KITTIDatasetReader(object):
    ...
    """
    kitti_00_02 = StereoCamera.from_calib_params(718.8560, 718.8560, 607.1928, 185.2157,
    ...
    p_bc = RigidTransform.from_rpyxyz(-np.pi / 2,
                                      0, -np.pi / 2, 0, 0, 0, axes='sxyz').inverse()
    ...
    def __init__(self, directory='',
        ...
        # Read stereo images
        seq_directory = os.path.join(
            os.path.expanduser(directory), 'sequences', sequence)
        self.stereo = StereoDatasetReader(directory=seq_directory,
                                          left_template=os.path.join(
                                              seq_directory, left_template),
                                          right_template=os.path.join(
                                              seq_directory, right_template),
                                          start_idx=start_idx, max_files=max_files, scale=scale)

        # Read poses
        try:
            pose_fn = os.path.join(os.path.expanduser(
                directory), 'poses', ''.join([sequence, '.txt']))
...
class KITTIStereoGroundTruthDatasetReader(object):
    def __init__(self, directory, is_2015=False, scale=1.0):
        ...
        self.noc = ImageDatasetReader(template=os.path.join(
            os.path.expanduser(directory), noc_dir, '%06i_10.png'))
        self.occ = ImageDatasetReader(template=os.path.join(
            os.path.expanduser(directory), occ_dir, '%06i_10.png'))
...
class KITTIRawDatasetReader(object):
    ...
    def __init__(self, directory,
        ...
        try:
            ...
            oxts_fn = os.path.join(
                os.path.expanduser(directory), oxts_template)
            self.oxts_ = OXTSReader(
                oxts_format_fn, template=oxts_fn, start_idx=start_idx, max_files=max_files)
        except Exception as e:
